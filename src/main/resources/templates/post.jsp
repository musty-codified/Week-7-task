<%@ page import="lombok.*" %>
<%@ page import="com.example.weekseventask.models.Post" %>
<%--
  Created by IntelliJ IDEA.
  User: decagon
  Date: 01/11/2022
  Time: 03:01
  To change this template use File | Settings | File Templates.
--%>


<!-- post starts -->

  <%

  List<Post> postWithNoComments = UserPostDAO.getPosts();
    List<Post> postsWithComments = PostCommentDAO.getPostComments(postWithNoComments);
    List<Post> postsWithLikes = GetLikes.populatePostsWithLikesFromDB(postsWithComments);

    for(Post post : postsWithLikes) {%>
      <div class="post">
        <div class="post__top">
          <img
                  class="user__avatar post__avatar"
                  src="data:image/png;base64,"
                  alt=""
          />
          <div class="post__topInfo">
            <h3><%=post.getPostAuthor()%></h3>
            <p><%=post.getDateCreated()%></p>
          </div>
        </div>

          <div class="edit_post" id="edit_post-<%=post.getId()%>">
              <%@include file="edit_post.jsp"%>
          </div>


        <div class="post__bottom" >
            <p><%=post.getPostContent()%></p>
        </div>

        <div class="post__options">
<%--          <input type="hidden" class="postLikedId" id="userLikedId<%=post.getId()%>" value="<%=post.getId()%>">--%>

          <div class="like-click postLikes-<%=post.getId()%> post__option">
              <span class="material-icons"> thumb_up </span>
              <p><%=post.getPostLikes().size()%> Like</p>
            <div class="">
                <input type="hidden" class="userLikedId" id="userLikedId" value="<%=request.getSession().getAttribute("userID")%>">
            </div>
          </div>

<%--          <form action="" class="comment">--%>
<%--            <label for="comment-checkbox" id="" class="comment-toggle">--%>
<%--              <div class="post__option">--%>
<%--                <span class="material-icons"> chat_bubble_outline </span>--%>
<%--                <p>Comment</p>--%>
<%--              </div>--%>
<%--            </label>--%>
<%--            <input type="checkbox" id="comment-checkbox" class="comment-display--none">--%>
<%--          </form>--%>

        <% if (Integer.valueOf(post.getAuthorId()).equals(session.getAttribute("userID"))) { %>
            <div class="post__option post-edit edit_post-<%=post.getId()%>">
                <span class="material-icons"> edit </span>
                <p>Edit</p>
            </div>


          <div class="post-delete posts__options-<%=post.getId()%> post__option">
            <span class="material-icons"> delete </span>
            <p>Delete</p>
          </div>
          <% } %>
        </div>
      </div>
      <!-- Comments starts here -->
      <div class="" id="com-t">
        <%@ include file="comment.jsp" %>
      </div>
      <!-- Comments ends here -->
<%
   } %>
      <!-- post ends -->